...
type ProfileAction = {
  setToken: (token: userProfileState["token"]) => void;
  clearToken: () => void;
  isUserAuth: () => boolean;
  editUser: (user: userProfileState["user"]) => void;
};
export const useProfileStore = create(
  devtools(
    persist(
      immer<userProfileState & ProfileAction>((set, get) => ({
        token: undefined,
        user: null,
        ...
        editUser: (user: UserProfile | null) => {
          if (user !== null)
            set((state) => {
              if (!state.user) state.user = {} as UserProfile;
